...
export const KanbanForm = ({
  ...
  function handleTaskKanbanSubmit({
    column,
    taskName,
    description,
    subTasks,
  ...
  return (
    <Dialog>
      <DialogTrigger className="text-xl text-start text-white bg-slate-900/90 px-3 min-h-12 min-w-12 rounded-full mb-3 flex justify-center items-center gap-3 group">
        {/* Icon */}
        <Plus />
      ...
      <DialogPortal>
        <DialogContent className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-slate-800 min-h-80 w-1/2 p-5 rounded-sm overflow-hidden">
          {/* Dialog Close */}
          <DialogClose className="absolute top-0 right-0 p-1 bg-slate-900">
            <X color="white" />
          </DialogClose>
          ...
          >
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                placeholder={
                  errors.taskName ? "Preencha o campo" : "Title for task..."
                }
                {...register("taskName")}
              />
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              >
                {selectValues.map((value, idx) => (
                  <option className="bg-zinc-900" key={idx} value={value}>
            ...
            <div className="flex flex-col gap-2">
              ...
              {/* TextAreat */}
              <textarea
                className="resize-none bg-black/40 h-36 p-1 rounded-sm outline-none"
                ...
                {subTasks.length > 0 && (
                  <ul className="h-20 flex flex-col gap-2">
                    {subTasks.map((task, index) => (
                      <li
                        key={index}
                        className="bg-black/20 p-2 rounded-sm flex items-center justify-between"
                      >
                        {/* Task name */}
                        <h1>{task.name}</h1>
            ...
            {/* Button submit */}
            <button
              className="bg-black/40 w-auto rounded-sm h-10"
              type="submit"
            >
