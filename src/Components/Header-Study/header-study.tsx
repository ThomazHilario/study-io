...
export const HeaderStudy = ({ isLoading }: { isLoading: boolean }) => {
  ...
  function changeFullScreen() {
    ...
    localStorage.setItem(
      "isFullScreen",
      JSON.stringify(isFullscreen ? false : true),
    );

    // Salvar valor do isResizable na localStorage
    localStorage.setItem(
      "isResizable",
      JSON.stringify(resizable === false ? true : false),
    );
  ...
  return (
    <header
      className={cn(
        "flex items-center justify-between py-1 px-3 bg-[#202224] basis-13 border-b-[1px] border-b-gray-600",
        isFullscreen ? "mt-6 pt-1" : "mt-0",
      )}
    >
      {/*  what user using app */}
