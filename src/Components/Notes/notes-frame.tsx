...
export const NotesFrame = memo(
  ({ notesList, setNotesList }: NotesFrameProps) => {
    const { isDragging, updateCheckedValue } = useDrag({
      key: "isDraggingNotesInLocalStorage",
    });
    ...
    function editNotes(noteId: string) {
      try {
        ...
        const seachNote = notesList.find(
          (note) => note.id === noteId,
        ) as NotesProps;
    ...
    return (
      <Drag
        nameDragComponent="notesFrameDrag"
        positionXDefault={10}
        positionYDefault={125}
        ...
        <section className="bg-slate-700 rounded-sm w-full cursor-pointer py-3">
          ...
          {isAddNote ? (
            <section className="px-2">
              ...
              <button
                className="w-full bg-red-500 rounded-sm mt-1 text-white"
                onClick={cancelNote}
              >
                Cancelar
              </button>
            ...
            <section className="px-2 w-[330px]">
              <button
                className="w-full rounded-sm border-2 text-center text-white"
                    ...
                    return (
                      <Note
                        ...
                        updateStateEditNotes={() =>
                          updateStateEditNotes(note.item)
                        }
                      />
                    );
                  })}
