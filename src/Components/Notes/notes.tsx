...
export const Note = ({
  ...
  return (
    <Dialog>
      <DialogTrigger className="p-1 bg-slate-900/30 rounded-sm text-white">
        <p className=" px-1 text-justify h-6 overflow-hidden">{note.item}</p>
      ...
      <DialogPortal>
        <DialogContent className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
          <section className="bg-slate-700 p-2 pt-8 rounded-sm text-white text-justify w-[30vw] overflow-hidden">
            <DialogClose className="absolute p-1 right-0 top-0 bg-gray-800 rounded-sm">
            ...
            {isEditNote ? (
              <textarea
                className="p-1 resize-none outline-none bg-black/20 w-full rounded-sm"
                rows={3}
                value={editNote}
                onChange={(e) => setEditNote(e.target.value)}
              />
            ) : (
              <article className="bg-black/40 p-2">
                <p>{note.item}</p>
              </article>
            )}
            ...
            <article className="mb-4 mt-4 flex gap-3">
              {isEditNote ? (
                <>
                  <button
                    className="bg-green-500 p-1 px-3 rounded-sm"
                    onClick={editNotes}
                  >
                    Editar
                  </button>
                  ...
                  <button
                    className="bg-slate-800 p-1 px-3 rounded-sm"
            ...
            {!isEditNote && <span>Criado há {noteDateCreate}</span>}
          </section>
        </DialogContent>
      </DialogPortal>
    </Dialog>
